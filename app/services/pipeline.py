...
def run_pipeline(
    ...
    if mode == "semantic":
        ...
        raw_results = search_fn(query_vec, top_k=top_k)
        # Normalize keys
        ...
        raw_results = hybrid_fn(search_query, top_k=top_k, candidate_k=10)
        for r in raw_results:
            raw_chunks.append({
                "id": r["id"],
                "text": r["text"],
        ...
        hybrid_fn = _get_hybrid_search()
        hybrid_candidates = hybrid_fn(search_query, top_k=10, candidate_k=10)
    ...
    return {
        ...
        "source_document": "InsurancePolicy.pdf",
